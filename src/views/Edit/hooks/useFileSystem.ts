...
export interface UseFileSystemProps {
  /** 可选的初始文件系统数据（从存储加载） */
  initialFileSystem?: StorageFileSystem | null;
}
 ...
 * 文件系统状态管理Hook
 */
export function useFileSystem(props?: UseFileSystemProps): UseFileSystemReturn {
  const { initialFileSystem } = props ?? {};

  const [nodes, setNodes] = useState<FileNode[]>(() => {
    if (initialFileSystem?.nodes?.length > 0) {
      return initialFileSystem.nodes.map(n => ({
        ...n,
  ...
  const [pinnedIds, setPinnedIds] = useState<string[]>(() => {
    return initialFileSystem?.pinnedIds ?? [];
  });

  const [explorerOrder, setExplorerOrder] = useState<string[]>(() => {
    if (initialFileSystem?.explorerOrder?.length > 0) {
      return initialFileSystem.explorerOrder;
    }
    return [DEFAULT_FILE_ID];
  });

  const [folderOrder, setFolderOrder] = useState<Record<string, string[]>>(() => {
    return initialFileSystem?.folderOrder ?? {};
  });
  ...
  useEffect(() => {
    if (hasInitialized.current || !initialFileSystem) return;
    hasInitialized.current = true;
    if (initialFileSystem.nodes?.length > 0) {
      setNodes(initialFileSystem.nodes.map(n => ({
        ...n,
        createdAt: typeof n.createdAt === "string" ? new Date(n.createdAt).getTime() : n.createdAt,
        updatedAt: typeof n.updatedAt === "string" ? new Date(n.updatedAt).getTime() : n.updatedAt,
      })) as FileNode[]);
  ...
  const nodesStructureKey = useMemo(
    () => nodes.map(n => `${n.id}:${n.type}:${n.parentId ?? "root"}`).sort().join("|"),
    [nodes],
  );
  ...
  useEffect(() => {
    setExplorerOrder(prev => {
      const rootIds = nodes
        .filter(n => (n.parentId === null || n.parentId === undefined) && !pinnedIds.includes(n.id))
        .map(n => n.id);
  ...
  useEffect(() => {
    setFolderOrder(prev => {
      ...
      for (const key of Object.keys(next)) if (!folderIds.includes(key)) delete next[key];
      for (const folderId of folderIds) {
        const childIds = nodes.filter(n => n.parentId === folderId).map(n => n.id);
  ...
  const pinnedNodes = useMemo(
    () => pinnedIds.map(id => nodes.find(n => n.id === id)).filter(Boolean) as FileNode[],
    [nodes, pinnedIds],
  );
