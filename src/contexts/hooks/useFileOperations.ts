import { createFileOnServer, saveFileContent, createFolderOnServer, deleteNodeOnServer, renameNodeOnServer, moveNodeOnServer } from "@/api/client";
import { FileNode, FileSystemAPI } from "@/types/filesystem";
import { useCallback, useMemo } from "react";
import { remapId } from "../FileOperationContext";
...
export function useFileOperations() {
    ...
    const createFile = useCallback(
        (
            ...
            // 调用后端创建文件
            createFileOnServer(parentId ?? "", fileName, content)
                .then((res) => {
                    if (res.id !== optimisticId) {
                        // 后端返回的 ID 与乐观 ID 不同，更新
                        setNodes((prev) =>
                            prev.map((n) =>
                                n.id === optimisticId ? { ...n, id: res.id } : n,
                            ),
                        );
                        if (parentId === null) {
                            setExplorerOrder((prev) =>
                                prev.map((id) => (id === optimisticId ? res.id : id)),
                            );
                        } else {
                            setFolderOrder((prev) => ({
                                ...prev,
                                [parentId]: (prev[parentId] ?? []).map((id) =>
                                    id === optimisticId ? res.id : id,
                                ),
                            }));
                        }
    ...
    const createFolder = useCallback(
        (name: string, parentId: string | null = null): string => {
            ...
            createFolderOnServer(parentId ?? "", name)
                .then((res) => {
                    if (res.id !== optimisticId) {
                        setNodes((prev) =>
                            prev.map((n) =>
    ...
    const deleteNode = useCallback(
        (id: string) => {
            ...
            collect(id);
            const remainingFiles = nodes.filter(
                (n) => n.type === "file" && !toDelete.has(n.id),
            );
            ...
            const updateIds = (oldPrefix: string, newPrefix: string) => {
                setNodes((prev) =>
                    prev.map((n) => {
                        ...
                        if (n.id.startsWith(oldPrefix + "/"))
                            return { ...n, id: newPrefix + n.id.slice(oldPrefix.length) };
                        if (n.parentId === oldPrefix) return { ...n, parentId: newPrefix };
                        if (n.parentId?.startsWith(oldPrefix + "/"))
                            return {
                                ...n,
                                parentId: newPrefix + n.parentId.slice(oldPrefix.length),
                            };
                        return n;
                    }),
                );
                ...
                setExplorerOrder((prev) =>
                    prev.map((e) =>
                        e === oldPrefix
                            ? newPrefix
                            : e.startsWith(oldPrefix + "/")
                                ? newPrefix + e.slice(oldPrefix.length)
                                : e,
                    ),
                );
                setFolderOrder((prev) => {
                ...
                if (
                    activeFileId === oldPrefix ||
                    activeFileId.startsWith(oldPrefix + "/")
                ) {
                    setActiveFileId(
                        activeFileId === oldPrefix
                            ? newPrefix
                            : newPrefix + activeFileId.slice(oldPrefix.length),
                    );
                }
            };
                ...
                prev.map((n) => {
                    ...
                    if (n.id.startsWith(id + "/"))
                        return {
                            ...
                            parentId:
                                n.parentId === id ? newId : remapId(n.parentId!, id, newId),
                        };
                    return n;
                }),
            );
